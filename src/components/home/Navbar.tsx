        ...
        >
            ...
            >
                ...
                <Link href="/" className="group flex flex-col focus:outline-none relative z-[60]">
                    <span className="font-heading text-sm sm:text-lg md:text-xl lg:text-2xl text-fg tracking-[0.05em] md:tracking-[0.2em] lg:tracking-[0.3em] font-medium transition-all duration-500 group-hover:scale-105 flex items-center uppercase whitespace-nowrap">
                ...
                <div className="flex items-center gap-2 md:gap-4 relative z-[60]">
                    ...
                    {/* CTA Button - Visible on all screens now */}
                    <button
                        onClick={() => window.scrollTo({ top: document.body.scrollHeight, behavior: 'smooth' })}
                        className={`relative overflow-hidden group font-montserrat text-[8px] sm:text-[10px] md:text-xs font-semibold tracking-widest uppercase transition-all duration-500 flex items-center justify-center
                            ${isScrolled && !isMobileMenuOpen
                                ? 'px-3 py-2 md:px-6 md:py-2.5 rounded-full bg-primary text-black hover:bg-white'
                                : 'px-4 py-2 sm:px-6 sm:py-2.5 md:px-8 md:py-3.5 border border-white/20 text-white hover:border-primary hover:bg-primary/10 rounded-sm'
                            }
                        `}
                    >
                        <span className="relative z-10 transition-colors duration-500">Inquire</span>
                        ...
                        <div className="relative z-10 min-h-screen flex flex-col items-center justify-center p-10 pt-32 pb-20">
                            <nav className="flex flex-col items-center gap-10 md:gap-12 w-full">
                                ...
                                <motion.div
                                    ...
                                    className="mt-12 pt-12 border-t border-white/10 w-full max-w-sm flex flex-col items-center gap-6"
                                >
                                    <div className="flex flex-col items-center gap-4 text-center">
                                        <p className="font-montserrat text-[10px] tracking-[0.4em] uppercase text-primary font-bold mb-2">Direct Contact</p>
                                        <a href={`tel:${siteConfig.contact.phone.replace(/\s+/g, '')}`} className="flex items-center gap-4 text-fg hover:text-primary transition-colors">
                                            <Phone size={18} className="text-primary" />
                                            <span className="font-heading text-lg tracking-widest uppercase">{siteConfig.contact.phone}</span>
                                        </a>
                                        <a href={`mailto:${siteConfig.contact.email}`} className="text-muted hover:text-white transition-colors text-xs font-montserrat tracking-wider">
                                    ...
                                    <div className="flex gap-8 mt-4">
                                        {[
                                            ...
                                            <div key={social.name} className="flex flex-col items-center gap-2 group cursor-pointer w-12">
                                                <div className="w-10 h-10 rounded-full border border-white/10 flex items-center justify-center text-muted group-hover:text-primary group-hover:border-primary group-hover:bg-primary/5 transition-all duration-300">
                                                    {social.name}
                                                </div>
                                                <span className="text-[7px] tracking-widest uppercase text-muted/40 group-hover:text-primary/60 transition-colors">{social.label}</span>
                                            </div>
                                        ))}
                                    </div>
                                </motion.div>
